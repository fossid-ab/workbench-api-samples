...
class TestArchitectureFunctions(unittest.TestCase):
    ...
    def create_scan_dataset(self, size: int):
        ...
        return {
            f"scan_{i:03d}": {"code": f"scan_{i:03d}", "name": f"Scan {i}"}
            for i in range(size)
        }
    ...
    def test_small_dataset_passthrough(self):
        ...
        client = MagicMock()
        result = find_old_scans(scans, client, 365)
    ...
    def test_process_scans_age_filtering(self, mock_batch):
        ...
        def mock_response(_client, scan_codes):
            results = {}
            for i, code in enumerate(scan_codes):
                date = "2022-01-01 10:00:00" if i < 2 else "2026-01-01 10:00:00"
                results[code] = {
                    "name": f"Test {code}",
                    "created": date,
                    "updated": date,
        ...
        result = process_scans(scans, client, 365)
...
class TestPlanManagement(unittest.TestCase):
    ...
    def test_create_scan_plan(self):
        ...
        self.assertEqual(plan[0]["project_code"], "PROJECT1")
...
class TestAPIFunctions(unittest.TestCase):
    """Test SDK-backed API helpers."""

    @patch('archive_stale_scans.list_scans_paginated')
    def test_list_scans(self, mock_paginated):
        """Test scan listing delegates to paginated SDK helper."""
        mock_paginated.return_value = {
            "scan1": {"code": "scan1", "name": "Scan 1"},
            "scan2": {"code": "scan2", "name": "Scan 2"},
        }
        client = MagicMock()

        result = list_scans(client)
        self.assertEqual(len(result), 2)
        self.assertIn("scan1", result)
        mock_paginated.assert_called_once()

    @patch('archive_stale_scans.get_scan_info')
    def test_get_scan_info_batch(self, mock_get_scan_info):
        """Test batch scan info retrieval."""
        mock_get_scan_info.return_value = {
            "name": "Test Scan",
        ...
        result = get_scan_info_batch(client, ["scan1", "scan2"])
        self.assertEqual(len(result), 2)
        self.assertIn("scan1", result)
